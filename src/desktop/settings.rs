...
use crate::{
    desktop::validation::{
        validate_bind_addr, validate_bundle_id, validate_max_depth, validate_max_nodes,
        validate_mcp_path, validate_process_name, validate_stock_service_url,
    },
    storage::Storage,
...
/// Load settings from SQLite, returning a public view that never contains secrets.
...
/// Validate and persist ordinary settings. Never touches secrets.
/// Returns the saved public view plus restart-required flag.
pub fn save_public_settings(
    ...
    // Validate all fields first, fail fast without partial writes.
    let bind_addr =
        validate_bind_addr(&request.bind_addr).map_err(|e| format!("bind_addr: {e}"))?;
    ...
    // Persist atomically in a single transaction; on partial failure no settings are half-applied.
    {
    ...
    // Determine restart_required: settings that affect running server/inspector
